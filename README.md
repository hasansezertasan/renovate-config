...
> **Auto-merge is green-only and scoped:** a matching PR merges automatically only after all its checks pass. Direct (runtime) dependencies and Python version bumps are **never** auto-merged — the owner reviews those manually. Override any rule per-repo via `packageRules` (see *Per-repo overrides*).
